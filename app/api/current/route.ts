...
// ---- SFI PARSER (from daily-solar-indices.txt) ----
// Format example:
// 2025 01 15  145  143  12  75 ...
// Columns: YEAR MONTH DAY  F107obs  F107adj  sunspot#  A-index ...
function parseLatestSfi(text: string): { sfi: number; prev: number } | null {
  const lines = text
    .split("\n")
    .map(l => l.trim())
    .filter(l => /^\d{4}\s+\d{2}\s+\d{2}/.test(l)); // only data rows
  ...
  const lastF = Number(last[3]); // F107 observed
...
// ---- Kp PARSER (from daily-geomagnetic-indices.txt) ----
